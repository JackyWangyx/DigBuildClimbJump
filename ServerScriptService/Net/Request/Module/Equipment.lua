...
function LoadInfo(player)
	local saveInfo = PlayerPrefs:GetModule(player, "Equipment")
	return saveInfo
end

function Equipment:ShowEquipment(player)
	local equipmentServerHandler = require(game.ServerScriptService.ScriptAlias.EquipmentServerHandler)
	equipmentServerHandler:ShowEquipment(player)
end

function Equipment:HideEquipment(player)
	local equipmentServerHandler = require(game.ServerScriptService.ScriptAlias.EquipmentServerHandler)
	equipmentServerHandler:HideEquipment(player)
end

function Equipment:GetPackageList(player)
	local saveInfo = LoadInfo(player)
	local packageList = saveInfo.PackageList
	local dataList = ConfigManager:GetDataList("Equipment")
...
function Equipment:Buy(player, param)
	...
	local data = ConfigManager:GetData("Equipment", id)
	-- 未解锁
	...
	if buyOrder > 1 then
		...
		local previousData = ConfigManager:SearchData("Equipment", "BuyOrder", previousOrder)
	...
	if buyOrder > 1 then
		...
		local nextData = ConfigManager:SearchData("Equipment", "BuyOrder", nextBuyOrder)
		if nextData then
	...
	AnalyticsManager:Event(player, AnalyticsManager.Define.BuyEquipment, "EquipmentID", data.ID)
	
	PlayerRecord:AddValue(player, PlayerRecord.Define.TotalGetEquipment, 1)
	EventManager:Dispatch(EventManager.Define.QuestGetEquipment, {
		Player = player,
		Value = 1,
	})
	
	EventManager:Dispatch(EventManager.Define.GetEquipment, {
		Player = player,
		ID = id,
	})
...
function Equipment:Get(player, param)
	local id = param.ID
	local packageList = Equipment:GetPackageList(player)
	local info = packageList[id]
...
function Equipment:Equip(player, param)
	...
	EventManager:Dispatch(EventManager.Define.RefreshEquipment, player)
	EventManager:Dispatch(EventManager.Define.RefreshPlayerProperty, player)
	return true
end

function Equipment:UnEquip(player, param)
	local packageList = Equipment:GetPackageList(player)
...
function Equipment:GetEquip(player)
	local packageList = Equipment:GetPackageList(player)
	for _, info in ipairs(packageList) do
...
function Equipment:CheckExist(player, param)
	local id = param.ID
	local packageList = Equipment:GetPackageList(player)
	for _, info in ipairs(packageList) do
