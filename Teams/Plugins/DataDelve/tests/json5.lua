...
function testDecodes(name: string, source: string, expected: json5.Json, options: TestDecodesOptions?)
	...
	if result.kind == "failure" then
		...
		if not equals(result.json, expected) then
			...
			reportInfo(
				`GOT:{json5.encode(result.json, { mode = "readable", tab = "  " })}\nEXPECTED:{json5.encode(
					expected,
					{ mode = "readable", tab = "  " }
				)}`
			)
...
testEncodesReadable(
	"encodes object keys in sorted order",
	{ z = 123, b = 123, a = 123, gh = 123, abc = 123 },
	[[{
  a: 123,
  abc: 123,
  b: 123,
...
testEncodesReadable("does not escape keys that are identifiers", { test = 5 }, [[{
  test: 5,
}]])
testEncodesReadable("escapes keys that aren't identifiers", { ["test::dog"] = "hey" }, [[{
  "test::dog": "hey",
}]])
testEncodesReadable("escapes keys that aren't identifiers 2", { ["test dog"] = "hey" }, [[{
  "test dog": "hey",
}]])
testEncodesReadable("escapes keys that aren't identifiers 3", { ["5test"] = "hey" }, [[{
  "5test": "hey",
}]])
...
testEncodesReadable("encodes short arrays on one line 2", { 1 }, "[1]", {
	assertOneline = true,
})
testEncodesReadable("encodes short arrays on one line 3", { "hey" }, "[\"hey\"]", {
	assertOneline = true,
})
testEncodesReadable("encodes short arrays on one line 4", { true }, "[true]", {
	assertOneline = true,
})
testEncodesReadable("encodes empty objects on one line", { }, "{}", {
	assertOneline = true,
})
testEncodesReadable(
