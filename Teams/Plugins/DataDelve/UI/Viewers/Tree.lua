...
local BufferWidget = require(script.Parent.Parent.Widgets.BufferWidget)
...
local TimestampHelper = require(Utilities.TimestampHelper)
local promptKeyValueForm = require(Utilities.promptKeyValueForm)
local getHighlightColors = require(Utilities.getHighlightColors)
local lockObjectWithoutResize = require(Utilities.lockObjectWithoutResize)
local checkTrulyVisible = require(Utilities.checkTrulyVisible)
local byteLengthToReadableString = require(Utilities.byteLengthToReadableString)
local showShadowOnScroll = require(Utilities.showShadowOnScroll)
...
local SEARCH_BOTTOM_MARGIN = 40
local NOT_SEARCHED_TRANSPARENCY = 0.6
...
function Tree.new(
	...
	-- setup instances
	local frame = script.TreeScrollingFrame:Clone()
	local branchContainer = script.BranchContainer:Clone()
	local shadow = script.Shadow:Clone()
	branchContainer.Size = UDim2.new(1, -frame.ScrollBarThickness, 1, 0)
	
	frame.Parent = container
	branchContainer.Parent = container
	...
	local self = setmetatable({
		...
		showKeyInfo = if options.showKeyInfo == nil then true else options.showKeyInfo,

		_colors = getHighlightColors(theme),

		_inModal = false,
		_modalState = options.modalState,
		_inRightClick = false,
		...
		_isSearching = false,
		_searchTask = nil,
		_originalVisibleIndexes = nil,
		_isSearchDirty = false,
		_searchedIndexes = nil,
		_searchTerm = nil,
		_noResultsLabel = nil,
	...
	self._searchResultsChanged = Instance.new("BindableEvent")
	self.searchResultsChanged = self._searchResultsChanged.Event
	...
	self._connections = {
		...
		showShadowOnScroll(frame, shadow),
	...
	if options.inputReceiver then
		table.insert(self._connections, options.inputReceiver.InputBegan:Connect(function(input: InputObject, gpe: boolean?)
			if gpe then return end
			if not checkTrulyVisible(self.frame) then
				return
			end
			if self._modalState.isInside or self._inRightClick then
				return
			end
...
function Tree:getDirtyCapability()
	return self
end
	
function Tree:getHistoryCapability()
	return self
end

function Tree:getKeyInfoCapability()
	return self
end
...
function Tree:_initializeBranchesFromData(data: JSONHelper.JSONValue, keyInfo: Types.KeyInfo, allOpen: boolean?)
	if self.showKeyInfo then
		...
		self._visibleIndexes = { 1 }
		self._flatDepths = dataDepths
		self._flatBranches = dataBranches
		return 1
	end
end
...
local function searchVisibleIndex(visibleIndexes: {number}, index: number): number?
	if #visibleIndexes == 0 then
		return nil
	end

	local lo = 1
	local hi = #visibleIndexes
	while lo < hi do
		local mid = lo + (hi - lo) // 2
		if visibleIndexes[mid] == index then
			return mid
		elseif visibleIndexes[mid] < index then
			lo = mid + 1
		else
			hi = mid
		end
	...
	if visibleIndexes[lo] == index then
		return lo
	end
end

function Tree:_getRootIndex(): number
	if not self.showKeyInfo then
		return 1
	else
...
function Tree:_getAmountOfUserIds(): number
	if not self._userIdsIndex then
		return 0
	else
...
function Tree:_canDeleteBranch(index: number): boolean
	local branch = self._flatBranches[index]
	return not (branch.LOCKVALUE or branch.LOCKKEY or branch.ISROOT)
end

function Tree:_canInsertAfter(index: number): boolean
	local branch = self._flatBranches[index]
	local depth = self._flatDepths[index]
	return depth >= 1 and not branch.LOCKVALUE
end

function Tree:_canInsertInto(index: number): boolean
	local branch = self._flatBranches[index]
	return ((branch[2] == ARRAY_SENTINEL or branch[2] == OBJECT_SENTINEL) and not branch.LOCKVALUE)
		or branch.ISUSERIDS
...
function Tree:_bumpVisibleIndexesAfter(index: number, bumpAmount: number, precomputedVisibleIndex: number?)
	local visibleIndex = precomputedVisibleIndex or searchVisibleIndex(self._visibleIndexes, index)
...
function Tree:_tryInsertInto(options: {
	...
	local visibleIndex = searchVisibleIndex(self._visibleIndexes, index)
	if visibleIndex then
		if branch[3] then
			if visibleIndex then
...
function Tree:_tryInsertAfter(options: {
	...
	local insertingAtVisibleIndex = searchVisibleIndex(self._visibleIndexes, insertAt)
...
function Tree:_deleteBranch(options: { index: number, onlyChildren: boolean?, pushAction: boolean })
	...
	if branch[2] == ARRAY_SENTINEL or branch[2] == OBJECT_SENTINEL then
		...
		if hasChildren then
			...
			local startVisibleIndex = searchVisibleIndex(self._visibleIndexes, index)
			local endVisibleIndex = nil
			local endIndex = nil
			for i = index, len do
				local depth = self._flatDepths[i]
				if depth <= startDepth then
					endVisibleIndex = searchVisibleIndex(self._visibleIndexes, i)
	...
	if deleteSelf then
		...
		local visibleIndex = searchVisibleIndex(self._visibleIndexes, originalIndex)
		if visibleIndex then
...
function Tree:_insertForUndoRedo(
	...
	if result.kind == "error" then
		...
		self:_focusBranch(result.index)
		self:_redrawBranches()
	end
...
function Tree:_moveArrayElementForUndoRedo(fromIndex: number, toIndex: number)
	...
	local branchVisibleIndex = searchVisibleIndex(self._visibleIndexes, branchIndex)
	if not branchVisibleIndex then
		self:_digExpand(branchIndex)
		
		branchVisibleIndex = searchVisibleIndex(self._visibleIndexes, branchIndex)
		if not branchVisibleIndex then
			return
		end
	end
	...
	if toIndex > #self._flatBranches then
		...
		local moveBeforeVisibleIndex = searchVisibleIndex(self._visibleIndexes, moveBeforeIndex)
...
function Tree:_scrollTo(visibleIndex: number)
	...
	local bottomMargin = if self._isSearching then SEARCH_BOTTOM_MARGIN else 0
	if self.frame.CanvasPosition.Y + self.frame.AbsoluteSize.Y - bottomMargin < scrollTo + SIZE_PER_BRANCH then
		self.frame.CanvasPosition = Vector2.new(0, scrollTo + SIZE_PER_BRANCH - self.frame.AbsoluteSize.Y + bottomMargin)
...
function Tree:_focusBranch(index: number)
	if self._isSearching then return end
...
function Tree:_digExpand(index: number)
	...
	for i = index, 1, -1 do
		...
		if depth == 0 then
			-- this might be the case in a search...
			-- just make it visible since the search will throw away the result anyways
			if self._isSearching and (not self._visibleIndexes[i]) then
				table.insert(self._visibleIndexes, i)
			end
...
function Tree:_rebuildVisibleIndexes(preallocationSize: number)
	self._visibleIndexes = table.create(preallocationSize)
	
	local visibleStack = {true}
	local currentDepth = 0
	for i, branch in self._flatBranches do
		local depth = self._flatDepths[i]
		if depth < currentDepth then
			for i = depth + 1, currentDepth do
				table.remove(visibleStack)
			end
		end
		
		local isVisible = visibleStack[#visibleStack]
		if isVisible then
			table.insert(self._visibleIndexes, i)
			
			if branch[3] ~= nil then
				currentDepth = depth + 1
				table.insert(visibleStack, branch[3])
			end
		end
	end
end
...
function Tree:_updateCanvasSize()
	self.frame.CanvasSize =
		UDim2.fromOffset(0, SIZE_PER_BRANCH * #self._visibleIndexes + (self._selectingBranchOffset or 0) + (self._isSearching and SEARCH_BOTTOM_MARGIN or 0))
end
...
function Tree:_updateBranchFrame(frame, branch, index)
	...
	self:_updateBranchFrameSelectStyle(index, frame)
	...
	if not processed then
		...
		elseif typeof(value) == "buffer" then
			frame.Value.Text = `<buffer {byteLengthToReadableString(buffer.len(value))}>`
...
function Tree:_promptEditBuffer(options: EditBufferOptions): buffer?
	local resultEvent = Instance.new("BindableEvent")
	
	local copy = buffer.create(buffer.len(options.buffer))
	buffer.copy(copy, 0, options.buffer)
	
	local bufferWidget = BufferWidget.new(self.theme, options.name, copy)
	
	local editingFrame = script.EditingBufferPage:Clone()
	local editingWrapper = StyleStateWrapper.new(self.theme, {
		background = BackgroundStyleState.from(self.theme, editingFrame),
		cancel = ButtonStyleState.from(self.theme, editingFrame.Cancel),
		label = LabelStyleState.from(self.theme, editingFrame.TextLabel),
	})
	editingFrame.Position = UDim2.fromOffset(options.position.X, options.position.Y)

	local modal = PopupHelper.modal(editingFrame, self.gui)
	PopupHelper.clamp(editingFrame, self._gui)

	editingWrapper.styleStates.cancel.button.Activated:Connect(function()
		resultEvent:Fire(nil)
	end)

	bufferWidget.interacted:Connect(function(submit)
		if submit then
			resultEvent:Fire("submit")
		else
			resultEvent:Fire(nil)
		end
	end)
	
	if options.onInitialize then
	...
	if not bufferWidget.dead then
		shouldSubmit = resultEvent.Event:Wait()
	else
		shouldSubmit = false
	end
	
	modal:destroy()
	editingWrapper:destroy()
	editingFrame:Destroy()

	-- Wait for result instruction then act on it
	if shouldSubmit then
		local copy = bufferWidget:getFreshBuffer()
		bufferWidget:destroy()
		return copy
	end
	...
	local function updateValueInput()
		...
		if valueType == "string" then
			defaultValues["string"] = defaultValues["string"]
				or (if typeof(lastValue) == "number" then tostring(lastValue) else "")
		...
		elseif valueType == "buffer" then
			...
			local frame = script.EditMenuInputTemplates.Buffer:Clone()
			local styleState = StyleStateWrapper.new(self.theme, {
				frame = BackgroundStyleState.from(self.theme, frame), -- just so it gets destroyed...
				label = LabelStyleState.from(self.theme, frame.BufferLabel),
				button = ButtonStyleState.from(self.theme, frame.Link, { style = "secondary" }),
			}, {
				autoFollowTheme = false,
			})
			wrapper.styleStates.value = styleState
			frame.BufferLabel.Text = byteLengthToReadableString(buffer.len(defaultValues["buffer"]))
			frame.Parent = editFrame.Form.Value

			frame.Link.Activated:Connect(function()
				if isEditingBuffer then return end
				
				isEditingBuffer = true
				
				local newBuffer = self:_promptEditBuffer({
					name = options.key,
					position = options.position,
					buffer = defaultValues["buffer"],
				...
				if newBuffer then
					lastValue = newBuffer
					defaultValues["buffer"] = newBuffer
					frame.BufferLabel.Text = byteLengthToReadableString(buffer.len(newBuffer))
				end
				
				if editFrame.Parent then
					wrapper.styleStates.unexpand.button.Visible = options.showUnexpand
					editFrame.Form.Visible = true
					editFrame.Title.Visible = true
				end
...
function Tree:_promptEditValue(index: number)
	...
	local userResponse, newValue, newValueIsArray

	self._modalState:wrap(function()
		if typeof(branch[2]) == "string" and branch[2]:find("\n") then
			-- Read-only not supported so abort.
			if locked then
				return
			end
		...
		elseif (typeof(branch[2]) == "string" and branch[2]:find("\n")) or (typeof(branch[2]) == "buffer") then
			-- Read-only not supported so abort.
			if locked then
				return
			end
			...
			userResponse = if newBuffer then "submit" else "cancel"
			newValue = newBuffer
			newValueIsArray = false
		else
			userResponse, newValue, newValueIsArray = self:_promptModalTextBox(frame, "value", {
	...
	-- this could be if they used the prompt edit menu which will edit on its own
	if not userResponse then
		return
	end
	...
	if result then
		...
		if insertResult.kind == "error" then
			...
			if not self._isSearching then
				self:_redrawBranches()
				self:_focusBranch(insertResult.index)
			end
		end
	end
end
...
function Tree:_initializeBranchFrame(frame, index: number)
	...
	local function tryStartDrag(input: InputObject)
		...
		if self._modalState.isInside and (not self._inRightClick) then
			return
		end
		if self.readOnly then
			return
	...
	local function onRightClick()
		...
		-- hack since you can open a context menu while one is still open
		if self._inRightClick then
			local start = os.clock()
			repeat task.wait()
			until (not self._inRightClick) or (os.clock() - start >= 0.5)
		end
		
		self:_setInRightClick(true)
		...
		if branch[2] == ARRAY_SENTINEL or branch[2] == OBJECT_SENTINEL then
			if (not self._isSearching) and self:_doesBranchHaveChildren(index) then
				table.insert(options, ContextMenuStyleState.SEPARATOR)
...
function Tree:_updateBranchVisible(index)
	...
	if not startVisibleIndex then
		warn("Branch opened that is not visible. How!?", debug.traceback())
		return
	end
	...
	if not self._flatBranches[index][3] then
		...
		if self._selectingBranchVisibleIndex then
			if
				startVisibleIndex < self._selectingBranchVisibleIndex
				and self._selectingBranchVisibleIndex <= (endVisibleIndex or #self._visibleIndexes)
			then
				self._selectingBranchVisibleIndex = nil
			elseif endVisibleIndex and self._selectingBranchVisibleIndex > endVisibleIndex then
				self._selectingBranchVisibleIndex -= endVisibleIndex - startVisibleIndex
			end
		end
		...
		if self._selectingBranchIndex and not self._selectingBranchVisibleIndex then
			self._selectingBranchVisibleIndex = searchVisibleIndex(self._visibleIndexes, self._selectingBranchIndex)
		end
	end
...
function Tree:_clearNonVisibleBranches()
	for index, frame in self._frames do
		if not searchVisibleIndex(self._visibleIndexes, index) then
...
-- The `forceRefresh` option is for things like search which mess with the
-- visibleIndexes and require refreshing all branches.
function Tree:_redrawBranches()
	...
	if self._selectingBranchVisibleIndex then
		...
		then
			...
			self.branchContainer.UIPadding.PaddingTop = UDim.new(0, startOffset - self.frame.CanvasPosition.Y)
		...
		elseif self.frame.CanvasPosition.Y > selectingBranchTop then
			...
			self.branchContainer.UIPadding.PaddingTop = UDim.new(0, startOffset - self.frame.CanvasPosition.Y + self._selectingBranchOffset)
		else
			self.branchContainer.UIPadding.PaddingTop = UDim.new(0, startOffset - self.frame.CanvasPosition.Y)
		end
	else
	...
	for visibleI = startVisibleIndex, endVisibleIndex do
		...
		if not i then
			-- May not exist if startVisibleIndex does not exist
			local endIndex = self._visibleIndexes[visibleI - 1]

			-- forceRefresh already has them all in the tempPool
			if endIndex then
				for i, frame in self._frames do
					if i > endIndex then
						table.insert(tempPool, { i, frame })
		...
		if not frame then
			...
			elseif #self._framePool > 0 then
				...
				--frame.Visible = true
				frame.Parent = self.branchContainer
			else
...
function Tree:_setInRightClick(inRightClick)
	self._inRightClick = not not inRightClick
	self.frame.ScrollingEnabled = not inRightClick

	if self._hoveringBranchIndex and (not inRightClick) then
		local hoveringIndex = self._hoveringBranchIndex
		self._hoveringBranchIndex = nil
		self:_updateBranchFrameSelectStyle(hoveringIndex)
	end
end

function Tree:_updateBranchFrameSelectStyle(index: boolean, frame: Frame?)
	frame = frame or self._frames[index]
	if frame then
		if self._selectingBranchIndex == index then
			frame.BackgroundColor3 = self._colors.selectBackground
			
			frame.Key.TextTransparency = 0
			frame.Value.TextTransparency = 0
		elseif self._hoveringBranchIndex == index then
			frame.BackgroundColor3 = self._colors.hoverBackground
			
			if self._isSearching and (not self._searchedIndexes[index]) then
				frame.Key.TextTransparency = NOT_SEARCHED_TRANSPARENCY
				frame.Value.TextTransparency = NOT_SEARCHED_TRANSPARENCY
			else
				frame.Key.TextTransparency = 0
				frame.Value.TextTransparency = 0
			end
...
function Tree:_setHovering(index: number?)
	...
	if index ~= lastIndex then
		...
		if lastIndex then
			self:_updateBranchFrameSelectStyle(lastIndex)
		end
		if index then
			self:_updateBranchFrameSelectStyle(index)
		end
	end
end
...
function Tree:_selectBranch(index: number, dontDeselect: boolean)
	...
	if index ~= lastIndex then
		...
		self._selectingBranchVisibleIndex = searchVisibleIndex(self._visibleIndexes, index)
		self._selectingBranchOffset = 0
...
function Tree:_updateColors()
	...
	if self._noResultsLabel then
		self._noResultsLabel.TextColor3 = self._colors.plain
	end
...
function Tree:_makeDirty()
	...
	if self._isSearching then
		self._isSearchDirty = true
		
		-- rebuild visible indexes since they are going to be messed up if search is active
		self:_startSearchTask()
	end
end
...
function Tree:getValue(): Types.GetValueResult
	local result = self:_unflatten(self:_getRootIndex())
	return {
		kind = "success",
		value = result.value.value,
		hasNil = result.hasNil,
	}
end
...
function Tree:tryDeleteSelected(): boolean
	...
	if not self:_canDeleteBranch(self._selectingBranchIndex) then
		return
	end
...
function Tree:trySelectNext(): boolean
	if (not self._selectingBranchIndex) or (not self._selectingBranchVisibleIndex) then
		return false
	end
...
function Tree:tryInsertSelected(): boolean
	...
	if branch and frame then
		if branch[2] == OBJECT_SENTINEL or branch[2] == ARRAY_SENTINEL then
			if self:_canInsertInto(self._selectingBranchIndex) then
				self:_promptInsertInto(self._selectingBranchIndex, frame.Value.AbsolutePosition)
			end
		else
			if self:_canInsertAfter(self._selectingBranchIndex) then
...
function Tree:getSearchCapability()
	return self
end

function Tree:_initSearch()
	self._isSearching = true
	self._isSearchDirty = false
	self._originalVisibleIndexes = self._visibleIndexes
	if self._selectingBranchIndex then
		self._originalSearchingBranch = self._flatBranches[self._selectingBranchIndex]
	end
	self._visibleIndexes = {}
	self._searchedIndexes = {}
end

function Tree:setSearch(searchTerm: string)
	if not self._isSearching then
		self:_initSearch()
	end
	...
	if searchTerm == "" then
		self:endSearch()
		self._searchResultsChanged:Fire({
			kind = "none"
		} :: Types.SearchResult)
	else
...
-- find "i" such that all for all 0 <= j <= i, visibleIndexes[j] <= index
local function searchVisibleIndexMinimum(visibleIndexes: {number}, index: number): number
	local lo = 1
	local hi = #visibleIndexes
	local result = 0
...
local ITERATIONS_PER_PAUSE = 15_000
local BUFFER_TEXT = "buffer"
function Tree:_startSearchTask()
	if self._searchTask then
		task.cancel(self._searchTask)
	end

	self._searchTask = task.spawn(function()
		self._visibleIndexes = {}
		self._searchedIndexes = {}
		...
		local addedBranches = {}
		local adding = table.create(3)

		local normalizedSearchTerm = self._searchTerm:lower()
		local iterations = 0
		local matches = 0
		
		local addedSelectedBranch = false
		local scrolledToSelectedBranch = false
		...
		local function update()
			...
			if addedSelectedBranch and (not scrolledToSelectedBranch) then
				scrolledToSelectedBranch = true

				local visibleIndex = searchVisibleIndex(self._visibleIndexes, self._selectingBranchIndex)
				if visibleIndex then
					self._selectingBranchVisibleIndex = visibleIndex
					self:_scrollTo(visibleIndex)
				end
			end
		end
		...
		for i, branch in self._flatBranches do
			local valueType = type(branch[2])
			local searched =
				tostring(branch[1]):lower():find(normalizedSearchTerm, 1, true)
				or (valueType == "string" and branch[2]:lower():find(normalizedSearchTerm, 1, true))
				or ((valueType == "boolean" or valueType == "number") and tostring(branch[2]):find(normalizedSearchTerm, 1, true))
				or ((valueType == "buffer") and BUFFER_TEXT:find(normalizedSearchTerm, 1, true))
			if searched then
				matches += 1
				table.insert(adding, i)
				self._searchedIndexes[i] = true

				-- dig back up
				local currentDepth = self._flatDepths[i]
				for j = i - 1, 1, -1 do
					local jDepth = self._flatDepths[j]
					if jDepth == currentDepth - 1 then
						if addedBranches[j] then
							break
						end

						currentDepth = jDepth
						table.insert(adding, j)
					end
				end

				while #adding > 0 do
					local got = table.remove(adding)
					table.insert(self._visibleIndexes, got)
					addedBranches[got] = true
					
					if got == self._selectingBranchIndex then
						addedSelectedBranch = true
					end
				end
			end

			iterations += 1
			if iterations == ITERATIONS_PER_PAUSE then
				iterations = 0
		...
		if #self._visibleIndexes == 0 then
			self:_ensureNoResultsLabel()
		else
			self:_deleteNoResultsLabel()
		end

		--print("search time", os.clock() - startTime)
		
		update()
	end)
end

function Tree:jumpPrevMatch()
	if not self._isSearching then return end
	if #self._visibleIndexes == 0 then return end
	local startIndex = searchVisibleIndexMinimum(self._visibleIndexes, self._selectingBranchIndex or 1)
	
	for visibleIndex = startIndex - 1, 1, -1 do
		local index = self._visibleIndexes[visibleIndex]
		if self._searchedIndexes[index] then
			self:_selectBranch(index, true)
			self:_scrollTo(visibleIndex)
			return
		end
	...
	for visibleIndex = #self._visibleIndexes, startIndex, -1 do
		local index = self._visibleIndexes[visibleIndex]
		if self._searchedIndexes[index] then
			self:_selectBranch(index, true)
			self:_scrollTo(visibleIndex)
...
function Tree:jumpNextMatch()
	if not self._isSearching then return end
	if #self._visibleIndexes == 0 then return end
	local startIndex = searchVisibleIndexMinimum(self._visibleIndexes, self._selectingBranchIndex or 1)
	
	for visibleIndex = startIndex + 1, #self._visibleIndexes do
		local index = self._visibleIndexes[visibleIndex]
		if self._searchedIndexes[index] then
			self:_selectBranch(index, true)
			self:_scrollTo(visibleIndex)
	...
	for visibleIndex = 1, startIndex do
		local index = self._visibleIndexes[visibleIndex]
		if self._searchedIndexes[index] then
			self:_selectBranch(index, true)
			self:_scrollTo(visibleIndex)
...
function Tree:endSearch()
	if self._searchTask then
		task.cancel(self._searchTask)
		self._searchTask = nil
	end
	...
	if self._isSearching then
		...
		self._searchedIndexes = nil
		self._searchTerm = nil
		
		if self._isSearchDirty then
			self._isSearchDirty = false
			
			-- good enough estimate size: #self._originalVisibleIndexes. Should pay for most allocations.
			self:_rebuildVisibleIndexes(#self._originalVisibleIndexes)
		else
			self._visibleIndexes = self._originalVisibleIndexes
		end
		self._originalVisibleIndexes = nil
		...
		if self._selectingBranchIndex then
			self:_focusBranch(self._selectingBranchIndex)
			self:_redrawBranches()
		end
	end
end

function Tree:_ensureNoResultsLabel()
	if not self._noResultsLabel then
		self._noResultsLabel = script.NoResults:Clone()
		self._noResultsLabel.TextColor3 = self._colors.plain
		self._noResultsLabel.Parent = self.frame
	end
end

function Tree:_deleteNoResultsLabel()
	if self._noResultsLabel then
		self._noResultsLabel:Destroy()
